...
function App() {
  ...
  const portAdmin = import.meta.env.VITE_BASE_PORT_ADMIN || Number()8081;
  const portClient = import.meta.env.VITE_BASE_PORT_CUSTOMER || 80;
  const queryClient = new QueryClient();

  // Kiểm tra port server
  const checkPort = () => {
    const port = window.location.port;
    return port == portAdmin ? "admin" : "client";
  };
