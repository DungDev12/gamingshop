...
const FiltersComponent = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      dispatch(addFilterNumber({ min: price.min, max: price.max }));
    }, 600);
  ...
  async function get() {
    try {
      const response = await axios.get(
        "http://localhost:8080/category/filters",
        {
          params: { category, brandItem: brand },
      ...
      <div className="my-[0.5rem] p-4 relative">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 px-2 py-4">
            {api &&
              api[0].filters.map((item) => {
                if (item.filterNumber) {
                  return (
                    <div key={item.id} className="w-full h-full">
                      <h5 className="text-[17px] mb-1 font-bold">
                        {item.title}
                      </h5>
